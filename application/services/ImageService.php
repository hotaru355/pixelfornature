...
class ImageService
{
    ...
    public function generateCoverPhotoCL($imageDimensions, $fbCover, $params)
    {
        $this->checkArguments($imageDimensions, $fbCover, $params);
        ...
        $command = sprintf(
                "convert \( -resize '%dx%d' -unsharp '0x0.75+0.75+0.008' -crop '%dx%d+%d+%d' %s \) %s " .
                        ...
                        $scaled ["y"], $params ["imagePath"], $this->logoPath, $labelText, $outputFile);
    ...
    private function checkArguments($imageDimensions, $fbCover, $params)
    {
